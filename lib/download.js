...
export async function expandArchive(archive, destination, strip = 1) {
  ...
  // The paths travel in the environment rather than in the command string.
  // Interpolating them into a single-quoted PowerShell literal breaks on a
  // path that contains a quote — `C:\\Users\\O'Brien\\…` closes the literal
  // early — and `-Path` treats its value as a *wildcard pattern*, so a path
  // with brackets in it (`C:\\build[1]\\…`) matches nothing at all.
  // `-LiteralPath` and `$env:` avoid both, and there is nothing left to quote.
  //
  // Silencing Write-Progress is not cosmetic: drawing it costs Windows
  // PowerShell more time than the unpacking does.
  await run(
    'powershell',
    [
      '-NoProfile',
      ...
      "$ProgressPreference = 'SilentlyContinue';" +
        ' Expand-Archive -LiteralPath $env:CZB_ARCHIVE' +
        ' -DestinationPath $env:CZB_STAGING -Force -ErrorAction Stop',
    ],
    {
      log: rootLog,
