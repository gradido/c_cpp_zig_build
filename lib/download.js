...
export async function extractArchive(archive, destination, options = {}) {
  ...
  if (plan.kind === 'expand-archive') {
    ...
    return expandArchive(archive, destination, strip)
  }
...
/**
 * Unpacks a zip with PowerShell, for a Windows that has no bsdtar to offer.
 *
 * Exported because this is the one place in the tool where PowerShell quoting
 * rules apply, and they are only testable on Windows: `chooseExtractor` reaches
 * for `System32\\tar.exe` first on anything from Windows 10 1803, so nothing
 * else would ever take this branch there.
 *
 * @param {string} archive
 * @param {string} destination
 * @param {number} [strip] leading directory levels to drop
 * @returns {Promise<string>} destination
 */
export async function expandArchive(archive, destination, strip = 1) {
  const staging = `${destination}.staging`
  // ExtractToDirectory creates the staging directory itself and refuses to
  // overwrite a file already there, so it gets a clean one, not an existing one.
  await fsp.rm(staging, { recursive: true, force: true })
  ensureDir(destination)

  // Two hazards, and .NET sidesteps both where `Expand-Archive` cannot.
  //
  // The paths travel in the environment rather than in the command string, so
  // a quote in one — `C:\Users\O'Brien\…` — has no literal to close. And
  // `ZipFile::ExtractToDirectory` takes plain strings: `Expand-Archive` reads
  // `-Path` as a wildcard pattern, and its `-DestinationPath` goes through
  // PowerShell's provider parsing too, where `[1]` in a directory name is a
  // character class. `-LiteralPath` fixes only the first of those; there is no
  // literal counterpart for the destination.
  //
  // Add-Type is for Windows PowerShell 5.1, where the assembly is not loaded
  // by default.
  await run(
    'powershell',
    [
      ...
      'Add-Type -AssemblyName System.IO.Compression.FileSystem;' +
        ...
        '$env:CZB_ARCHIVE, $env:CZB_STAGING)',
    ],
    {
      log: rootLog,
      env: { ...process.env, CZB_ARCHIVE: archive, CZB_STAGING: staging },
    },
  )

  // Zip has no equivalent of tar's --strip-components, so the extra levels
  // come off afterwards.
  let source = staging
 ...
 *   * one whose owner *is* running is waited on however old it is, because a
 *     slow first build legitimately holds it for a long time;
 *   * an age limit covers only the lock that names no pid at all;
...
export async function withLock(lockPath, work, options = {}) {
  ...
  for (;;) {
    try {
      ...
      // Age decides nothing while the owner can be asked. A first build on a
      // slow link holds this for as long as the download takes, which can be
      // longer than any age limit worth setting — and stealing it would put two
      // extractions in one directory. An owner that is gone is taken over at
      // once; one that is running is waited on, however old the lock is, and
      // the deadline below is what stops that being forever.
      const reclaim =
        !owner || (owner.pid === undefined ? owner.ageMs > staleAfterMs : !processAlive(owner.pid))
      if (reclaim) {
      ...
      if (!announced) {
        announced = true
        log.step(
          `waiting for another build (pid ${owner.pid ?? 'unknown'}) to finish; ` +
 ...
 *
 * `pid` is undefined when the file holds no usable one — a build killed between
 * creating it and naming itself. There is then nobody to ask whether the lock
 * is still wanted, which is the only case where its age decides.
 *
 * @returns {Promise<{ pid: number | undefined, ageMs: number } | undefined>}
 */
