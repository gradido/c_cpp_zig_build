...
export async function resolveZig(options = {}) {
  ...
  await withLock(
    path.join(cacheHome(), 'zig', `.${version}.lock`),
    async () => {
...
async function installZig(version, log, checkSignature) {
    ...
    for (const url of urls) {
      try {
        await downloadFile(url, archive, {
          sha256: build.shasum,
          label: filename,
          // The index knows the size, so the progress bar does not depend on
          // the mirror sending a Content-Length.
  ...
  // The progress bar erases itself when the download ends, and unpacking 50-90
  // MB takes long enough that the silence reads as a hang.
  log.step(`unpacking ${filename}`)
