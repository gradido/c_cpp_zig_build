...
export const ZIG_PACKAGE_NAME = 'c_cpp_zig_build'
...
export async function syncTemplate(root, dirName, log) {
  ...
  if (!isDirectory(source)) {
    throw new Error(`the c-cpp-zig-build package is incomplete: ${source} is missing`)
  }
...
async function writeReadme(destination) {
  ...
  const text = [
    '# Generated directory',
    '',
    'This is a copy of the Zig build template from the `c-cpp-zig-build` npm',
  ...
  if (!exists(zonPath)) {
    throw new Error(
      `no build.zig.zon in ${root}.\n` +
        'Run `npx c-cpp-zig-build init` to create the build files for this project.',
    )
  }
