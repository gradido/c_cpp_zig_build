...
import { downloadFile, fetchText, withLock } from './download.js'
 ...
 */
export async function resolveNodeHeaders(root = process.cwd()) {
  const pkg = resolveNodeApiHeaders(root)
  if (!pkg) {
    throw new Error("nodeHeaders is set to 'package' but `node-api-headers` is not installed. ")
  }
