 ...
 *
 *   c-cpp-zig-build              build the project in the current directory
 *   c-cpp-zig-build init         create build.zig, build.zig.zon and the rest
 *   c-cpp-zig-build clean        remove everything the build produced
 *   c-cpp-zig-build info         report what a build would use
 *   c-cpp-zig-build zig -- ...   run the managed Zig toolchain
 */
...
const BOOLEAN_FLAGS = {
  ...
  '--no-verify-signature': ['verifySignature', false],
  '--napi': ['napi', true],
  '--no-napi': ['napi', false],
  '--bun': ['bun', true],
  '--no-bun': ['bun', false],
...
function usage() {
  const lines = [
    '',
    c.bold('c-cpp-zig-build') + ' - build native C/C++ Node.js modules with Zig',
    '',
    c.bold('Usage'),
    '  c-cpp-zig-build [build] [options]     build the project (the default)',
    '  c-cpp-zig-build init [options]        create the build files',
    '  c-cpp-zig-build clean                 remove build output and caches',
    '  c-cpp-zig-build info                  show what a build would use',
    ...
    '  --no-verify-signature   skip the Zig archive signature check (not advised)',
    '',
    c.bold('init options'),
    ...
    '',
    c.bold('Examples'),
    '  c-cpp-zig-build init --language c++',
    '  c-cpp-zig-build --debug --verbose',
    '  c-cpp-zig-build --target x86_64-linux-gnu --target aarch64-macos',
    '  c-cpp-zig-build zig -- fmt --check build.zig',
    '',
  ]
  ...
  switch (command) {
    ...
    case 'zig':
      if (passthrough.length === 0) {
        throw new Error('nothing to pass to zig. Use: c-cpp-zig-build zig -- version')
      }
...
main().catch((error) => {
  ...
  if (process.env.C_CPP_ZIG_BUILD_DEBUG) {
