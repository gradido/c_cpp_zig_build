...
export async function init(options = {}) {
  ...
  log.raw(`  ${c.grey('npm i -D zig-native-build')}   ${c.grey('# or: bun add -d / yarn add -D')}`)
...
const znb = @import("${ZIG_PACKAGE_NAME}");

pub fn build(b: *std.Build) !void {
    _ = try znb.addStaticLibrary(b, .{ .name = "${name}" });
}
`
  }
  return `const std = @import("std");
...
pub fn build(b: *std.Build) !void {
    ...
    // output directory. See the zig-native-build README for the options.
    _ = try znb.addNodeAddon(b, .{ .name = "${name}" });
}
`
}
...
function buildZigZon(name) {
  return `.{
    ...
    .dependencies = .{
        // The build template, kept in sync by \`zig-native-build\`.
...
// zig-native-build, so this compiles as it stands. Declare it in package.json
...
function updatePackageJson(root, log) {
  ...
  for (const [key, value] of Object.entries({
    ...
    'build:clean': 'zig-native-build clean',
  })) {
    if (!pkg.scripts[key]) {
