...
import { resolveNodeAddonApi, resolveNodeApiHeaders, resolveNodeHeaders } from './node-headers.js'
...
export { defineConfig, loadConfigFile, resolveConfig }

/**
 * Builds the project.
 *
 * The Zig toolchain is the only thing downloaded, once, into `~/.zig-build`.
 * The Node-API headers come from `node-api-headers` and the Windows import
 * library is generated from its module definition file at build time, so
 * neither costs a download.
 *
...
export async function info(overrides = {}) {
  ...
  const rows = [
    ...
    [
      'zig signature',
      config.verifySignature ? 'verified against the Zig project key' : 'NOT CHECKED',
    ],
    ['zig cache', cacheHome()],
  ...
  if (config.napi) {
    ...
    const apiHeaders = resolveNodeApiHeaders(config.root)
    let headers
    try {
      headers = resolveNodeHeaders(config)
    } catch (err) {
      headers = { includeDir: `(unavailable: ${err.message})`, source: 'missing' }
    }
...
function describePackage(resolved) {
  ...
  const origin = resolved.declared ? 'declared by the project' : 'via c-cpp-zig-build'
...
async function prepare(config, log) {
  ...
  if (config.napi) {
    nodeHeaders = resolveNodeHeaders(config)
    addonApi = resolveNodeAddonApi(config.root)
    apiHeadersPackage = resolveNodeApiHeaders(config.root)
...
async function zigArgsFor(config, toolchain, target, outDir, log) {
  ...
  if (config.napi) {
    ...
    if (target.baseTriple.includes('windows')) {
      await addWindowsLinkArgs(args, config, toolchain, log)
    }
  }
 ...
 *
 * `node-api-headers` ships a module definition file for the Node-API exports,
 * and `zig dlltool` turns it into an import library at build time: a few
 * kilobytes, no download, and the only route that works for Bun, whose exports
 * live in `bun.exe` rather than `node.exe`. The template builds one import
 * library per runtime from the same file.
 */
async function addWindowsLinkArgs(args, config, toolchain, log) {
  const def = toolchain.apiHeadersPackage?.nodeApiDef
  if (!def || !exists(def)) {
    throw new Error(
      'building a Windows addon needs `node-api-headers`, whose node_api.def becomes the ' +
        'import library. It ships as a dependency of this package, so this usually means a ' +
