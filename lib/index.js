...
import { resolveNodeAddonApi, resolveNodeApiHeaders, resolveNodeLib } from './node-headers.js'
...
async function prepare(config, log) {
  ...
  if (config.napi) {
    ...
    if (apiHeadersPackage) {
      log.debug(`node headers: node-api-headers ${describePackage(apiHeadersPackage)}`)
    }
    if (addonApi) {
  ...
  return { zig: zigToolchain, addonApi, apiHeadersPackage }
}
...
async function zigArgsFor(config, toolchain, target, outDir, log) {
  ...
  if (config.napi) {
    if (!toolchain.apiHeadersPackage) {
      throw new Error(
        '`node-api-headers` could not be resolved, so there are no Node-API headers to ' +
          'compile against. Add it as a devDependency of the project.',
      )
    }
    args.push(`-Dnode-headers=${toolchain.apiHeadersPackage.includeDir}`)
