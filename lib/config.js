...
import { detectHostTriple } from './host.js'
...
export async function resolveConfig(overrides = {}) {
  ...
  return {
    ...
    /** A directory with node_api.h in it. Unset: use `node-api-headers`. */
    nodeHeaders: merged.nodeHeaders,
    ...
    /** Check the Zig archive's minisign signature. Off is a deliberate risk. */
    verifySignature: merged.verifySignature ?? true,
